...
| --- | --- | --- | --- | --- |
| [`async_dp_engine.py`](../../../afd_plugin/compat/patches/async_dp_engine.py): `EngineCoreProc.run_engine_core`, `vllm.v1.engine.utils.launch_core_engines` and its imported client alias, `DPCoordinatorProc.run_coordinator`, `DPCoordinatorProc.process_input_socket`, `DPAsyncMPClient.add_request_async`, `DPLBAsyncMPClient.get_core_engine_for_request` | Async-DP Attention selects regular `EngineCoreProc`, keeps asynchronous coordinator stats but disables DP wave coordination and `FIRST_REQ`. The opt-in prefill-token policy forwards versioned scalar debt and otherwise preserves request-count routing. | Imported by `register_afd`; general async bindings keep the target/dev guard, while the copied coordinator is exact-`0.26.0` only. Its plugin-owned process target reapplies the loop in spawn children; newer development versions retain their native coordinator. | [`test_async_dp_engine.py`](../../../tests/unit/compat/patches/test_async_dp_engine.py) covers spawn import, exact-version isolation, process selection, coordinator mode, stale debt, wakeup, and reload; [`test_async_dp_dplb_runtime.py`](../../../tests/unit/compat/patches/test_async_dp_dplb_runtime.py) covers native and token routing. | Remove when vLLM exposes role-selectable asynchronous DP scheduling and typed token-load stats/routing hooks. |
...
| [`config_validation.py`](../../../afd_plugin/compat/patches/config_validation.py): `EngineArgs.create_engine_config`, `VllmConfig.__post_init__` | For AFD-owned ubatching with a non-DeepEP backend, temporarily presents `deepep_low_latency` during upstream validation and restores the configured backend. After upstream platform normalization, maps an initial `worker_cls="auto"` to the role-specific worker. It also validates the opt-in prefill-token DPLB against the complete vLLM config. | Imported by `register_afd`; accepts the target version, development versions, or missing version metadata. Saves originals on upstream modules under AFD-specific attributes before installing wrappers. Explicit worker paths and non-AFD configs are not remapped. | [`test_config_validation.py`](../../../tests/unit/compat/patches/test_config_validation.py) covers backend relaxation and worker mapping; [`test_config.py`](../../../tests/unit/config/test_config.py) covers token-policy parsing and deployment constraints. | Remove branches as upstream gains plugin-owned ubatching, role-aware worker selection, and typed DPLB extension points. |
| [`engine_core.py`](../../../afd_plugin/compat/patches/engine_core.py): `EngineCore.__init__`, `_initialize_kv_caches`, `shutdown`; `EngineCoreProc.run_busy_loop`; `DPEngineCoreProc.run_busy_loop` | AFD FFN becomes a connector daemon with FFN-safe startup and shutdown. Async Attention uses the ordinary independent loop and publishes changed request counts; the opt-in policy additionally reports live prompt-token debt at bounded cadence. Non-FFN branches copy pinned upstream behavior. | Imported by `register_afd`; **no patch-local version guard and no saved-original sentinel**. Direct class assignment means the package pin and review discipline are the compatibility guard. | [`test_engine_core.py`](../../../tests/unit/compat/patches/test_engine_core.py) covers FFN lifecycle, non-FFN behavior, independent request-count publication, token progress, and mixed-workload invalidation. | Remove when vLLM offers headless connector-daemon lifecycle and typed asynchronous DP load-stat hooks. |
