...
| --- | --- | --- | --- | --- |
| [`async_dp_engine.py`](../../../afd_plugin/compat/patches/async_dp_engine.py): `EngineCoreProc.run_engine_core`, `vllm.v1.engine.utils.launch_core_engines` and its imported client alias, `DPCoordinatorProc.process_input_socket`, `DPAsyncMPClient.add_request_async`, `DPLBAsyncMPClient.get_core_engine_for_request` | Async-DP Attention selects regular `EngineCoreProc`, keeps asynchronous coordinator stats but disables DP wave coordination and `FIRST_REQ`. The opt-in prefill-token policy forwards versioned scalar debt and otherwise preserves request-count routing. | Imported by `register_afd`; accepts target-version prefix, development versions, or missing version metadata. Direct assignments; reload/idempotence behavior has focused tests. | [`test_async_dp_engine.py`](../../../tests/unit/compat/patches/test_async_dp_engine.py) covers process selection, coordinator mode, stale debt, wakeup, and reload; [`test_async_dp_dplb_runtime.py`](../../../tests/unit/compat/patches/test_async_dp_dplb_runtime.py) covers native and token routing. | Remove when vLLM exposes role-selectable asynchronous DP scheduling and typed token-load stats/routing hooks. |
