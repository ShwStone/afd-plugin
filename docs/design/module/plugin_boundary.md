...
| `attention_dplb_policy` | `request_count` | Selects native request-count DPLB or the experimental async Attention `prefill_token_sum` policy. It is runtime routing state and is excluded from the graph hash. |
