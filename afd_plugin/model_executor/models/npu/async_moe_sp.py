...
class AsyncMoeStageInputs:
    """TP-local tensors and token counts for the two global MoE stages."""

    hidden_states: list[torch.Tensor]
    residuals: list[torch.Tensor | None]
    positions: list[torch.Tensor]
    llama_4_scaling: list[torch.Tensor | None]
    local_input_token_counts: tuple[int, ...]
    local_actual_token_counts: tuple[int, ...]
...
def build_async_moe_stage_inputs(
    ...
    tp_rank = int(tp_group.rank_in_group)
    tp_size = int(tp_group.world_size)
    global_input_tokens = metadata.num_parent_input_tokens
    ...
    global_residual = (
        None
        if residual is None
        else _all_gather_rows(residual, expected_rows=global_input_tokens)
    )
    ...
    local_input_counts: list[int] = []
    local_actual_counts: list[int] = []

    for stage_slice, stage_actual_tokens in zip(
        metadata.ubatch_slices,
        metadata.stage_actual_token_counts,
        strict=True,
    ):
        stage_input_tokens = int(stage_slice.num_tokens)
        ...
        stage_hidden = _slice_and_pad_first_dim(
        ...
        stage_scaling.append(
            _slice_optional_token_tensor(
                stage_scaling_tensor,
                scaling_token_dim,
                local_stage_slice,
            ),
        )
        local_input_counts.append(local_stage_tokens)
        local_actual_counts.append(
            _local_actual_token_count(
                stage_actual_tokens=int(stage_actual_tokens),
                local_stage_tokens=local_stage_tokens,
                tp_rank=tp_rank,
            ),
        )
    ...
    return AsyncMoeStageInputs(
        ...
        local_input_token_counts=tuple(local_input_counts),
        local_actual_token_counts=tuple(local_actual_counts),
    )
...
def restore_async_moe_stage_outputs(
    ...
    if not metadata.use_sequence_parallel:
        return _pad_first_dim(
            ...
            metadata.num_parent_input_tokens,
        )
    ...
    global_stage_outputs = [
        _all_gather_rows(
            stage_output,
            expected_rows=int(stage_slice.num_tokens),
        )
...
def _build_replicated_stage_inputs(
    ...
    return AsyncMoeStageInputs(
        hidden_states=[
            _slice_and_pad_first_dim(
                hidden_states,
                stage.token_slice,
                int(stage.num_tokens),
            )
            for stage in metadata.ubatch_slices
        ],
        residuals=[
            (
                None
        ...
        local_input_token_counts=tuple(
            int(stage.num_tokens) for stage in metadata.ubatch_slices
        ),
        local_actual_token_counts=metadata.stage_actual_token_counts,
    )
...
def _trim_real_stage_outputs(
    ...
    for stage_output, stage in zip(
        ...
        expected_rows = int(stage.num_tokens)
...
def _slice_optional_token_tensor(
    ...
    if tensor is None or token_dim is None:
        return tensor
    return _slice_token_dim(tensor, token_dim, token_slice)
...
def _slice_and_pad_first_dim(
    ...
    sliced_tensor = tensor[token_slice]
    return _pad_first_dim(sliced_tensor, output_tokens)
...
def _local_actual_token_count(
    *,
    stage_actual_tokens: int,
    local_stage_tokens: int,
    tp_rank: int,
) -> int:
    local_start = tp_rank * local_stage_tokens
    local_stop = local_start + local_stage_tokens
    return max(0, min(stage_actual_tokens, local_stop) - local_start)
