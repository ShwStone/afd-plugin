...
def test_prefix_sensitivity_disables_overlapping_native_warmups() -> None:
    ...
    assert command[-4:] == [
        ...
        "repeat=1",
        "prefix_ratio=0.25",
        "arrival_seed=20260817",
    ]


def test_arrival_seed_is_per_repeat_and_shared_across_systems() -> None:
    config = load_experiment_config(EXAMPLE_CONFIG_PATH)
    runs = build_runs(config, batch_tokens=[8192])

    seeds: dict[tuple[str, int], str] = {}
    for run in runs:
        command = build_benchmark_command(config, run)
        seed_value = command[command.index("--seed") + 1]
        key = (run.system.name, run.repeat)
        seeds[key] = seed_value

    assert seeds[("dp4_tp8_sp", 1)] == "20260817"
    assert seeds[("dp4_tp8_sp", 2)] == "20260818"
    assert seeds[("dp4_tp8_sp", 3)] == "20260819"
    # Same repeat index -> same seed for every system (paired replay).
    assert seeds[("afd_dp3_tp8_ep8", 1)] == seeds[("dp4_tp8_sp", 1)]
    assert seeds[("afd_dp3_tp8_ep8", 3)] == seeds[("dp4_tp8_sp", 3)]
