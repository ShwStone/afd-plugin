...
def test_stream_handler_decodes_utf8_split_across_chunks() -> None:
    handler = SimpleNamespace()
    streamed_response_handler_init(handler)
    message = (
        "data: "
        + json.dumps({"choices": [{"text": "你"}]}, ensure_ascii=False)
        + "\n\n"
    ).encode()
    split_index = message.index("你".encode()) + 1

    assert streamed_response_handler_add_chunk(handler, message[:split_index]) == []
    assert streamed_response_handler_add_chunk(
        handler,
        message[split_index:],
