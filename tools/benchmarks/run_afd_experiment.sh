#!/usr/bin/env bash
# Fork-ready runner for the AFD (afd_dp3_tp8_ep8) system only.
# Each fork agent points NODE0/NODE1 at its own pod pair.
#
# Usage:
#   NODE0=afd-exp-20260804-2 NODE1=afd-exp-20260804-3 \
#     bash tools/benchmarks/run_afd_experiment.sh [--resume] [--smoke]
set -euo pipefail
...
cell_complete() {
  ...
  for rps in 4 6 8 10 12; do
    ...
    # itask exec runs no shell; wrap `test -f` in bash -c.
...
run_group() {
  ...
  # Node0: attention master (all 16 devices)
  itask exec "$NODE0" --tty=false -- bash -c "
    ...
    setsid env MAX_NUM_BATCHED_TOKENS=$bt DP_START_RANK=0 VLLM_ENABLE_PREFIX_CACHING=$pc DP_ADDRESS=$NODE0_IP AFD_HOST=$NODE0_IP \
      bash tools/benchmarks/prefill_launch_afd_attention.sh \
      > bench_results/logs/${label}_node0.log 2>&1 < /dev/null &
  " 2>/dev/null || true
  sleep 10
  # Node1: attention headless (devices 0-7) + FFN (devices 8-15)
  itask exec "$NODE1" --tty=false -- bash -c "
    cd $REPO && mkdir -p bench_results/logs
    export PROMETHEUS_MULTIPROC_DIR=/a3_inference/itask/workdir/tq02357756/prometheus_tmp
    mkdir -p \$PROMETHEUS_MULTIPROC_DIR
    setsid env MAX_NUM_BATCHED_TOKENS=$bt DP_START_RANK=2 ATTN_DEVICES=0,1,2,3,4,5,6,7 VLLM_ENABLE_PREFIX_CACHING=$pc DP_ADDRESS=$NODE0_IP AFD_HOST=$NODE0_IP \
      bash tools/benchmarks/prefill_launch_afd_attention.sh \
      > bench_results/logs/${label}_node1_attn.log 2>&1 < /dev/null &
    sleep 3
    setsid env MAX_NUM_BATCHED_TOKENS=$bt VLLM_ENABLE_PREFIX_CACHING=$pc DP_ADDRESS=$NODE0_IP AFD_HOST=$NODE0_IP \
  ...
  if ! wait_server 900; then
    ...
    echo "[INFO] Node1 attn log tail:"
    itask exec "$NODE1" --tty=false -- tail -10 "$REPO/bench_results/logs/${label}_node1_attn.log" 2>/dev/null || true
    kill_all
    return 1
  fi
...
for bt in 8192 16384 32768 49152 65536; do
  for ratio in 0.25 0.5 0.75 0.9 0.95 0.99; do
    suffix="${ratio//./}"
    run_group "$bt" "$ratio" "1" "full_afd_bt${bt}_prefix${suffix}"
  done
done

echo ""
echo "=============================================="
echo "  AFD experiment complete!"
echo "=============================================="