#!/usr/bin/env bash
# Single-instance stock baseline (DP4TP4EP16, 16 cards, one pod), slow-rate sweep.
# mbt in {8192, 32768} x rate in {0.5x, 0.75x, 1x}; one server per mbt, three
# replays per server (queue drains fully between runs: client waits for its
# own completions, plus a settle sleep).
#
# Usage: POD=v4f-xnode-2 baseline1x_sweep.sh
set -uo pipefail
...
NASDIR=/a3_inference/itask/workdir/tq02357756/shwstone/baseline1x_results
...
say "== preflight =="
$S "$POD" -- bash -c "test -f /tmp/xnode32_cleanup.sh && test -f $BASE_LAUNCHER && ls $WL/formal_1_plan.json $WL/formal_1_slow2x_plan.json $WL/formal_1_slow1p33x_plan.json $WL/formal_1_requests.jsonl >/dev/null && echo PRE_OK" 2>&1 | tr -d '\r' | tail -1

run_plan() { # $1=mbt $2=plan file $3=output tag
  local MBT=$1 TAG=base1x_mbt${MBT}_$3
...
for MBT in 8192 32768; do
  TAG=base1x_mbt${MBT}
  say "== mbt=$MBT: cleanup + start baseline =="
  $S "$POD" -- bash /tmp/xnode32_cleanup.sh 2>&1 | tr -d '\r' | tail -2
  $S "$POD" -- bash -c "setsid bash -c 'env DP_SIZE=4 TP_SIZE=4 MAX_NUM_BATCHED_TOKENS=$MBT PORT=8000 ADDITIONAL_CONFIG=cws nohup bash $BASE_LAUNCHER > /tmp/${TAG}.log 2>&1 &' ; echo started" 2>&1 | tr -d '\r' | tail -1
  ...
  say "ready; CWS check:"
  $S "$POD" -- bash -c "grep -c 'Enabled DeepSeek-V4 persistent compressor tails' /tmp/${TAG}.log || true" 2>&1 | tr -d '\r' | tail -1
  ...
  $S "$POD" -- bash -c "
    mkdir -p $NASDIR
    cp /tmp/${TAG}_0p5x.json /tmp/${TAG}_0p75x.json /tmp/${TAG}_1x.json /tmp/${TAG}.log $NASDIR/
    echo archived" 2>&1 | tr -d '\r' | tail -1
done
say "== ALL_COMPLETE baseline1x =="
