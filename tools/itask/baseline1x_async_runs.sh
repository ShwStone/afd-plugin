#!/usr/bin/env bash
# Single-instance stock baseline (DP4TP4EP16, 16 cards, one pod), async-sched
# ON, rate sweep 0.5x -> 1.5x (0.25x steps) at mbt {8192, 32768}.
# Retest of baseline1x_sweep.sh with ASYNC_SCHEDULING=1 (the 2026-09-02
# baseline1x cells predate the async default flip); every other knob is
# identical to baseline1x_sweep.sh, so mbt is the only varying hyperparameter
# within a rate.
#
# Usage: POD=v4f-base-2 baseline1x_async_runs.sh
set -uo pipefail
...
NASDIR=/a3_inference/itask/workdir/tq02357756/shwstone/baseline1x_async
...
run_plan() { # $1=mbt $2=plan file $3=output tag
  local MBT=$1 TAG=base1xas_mbt${MBT}_$3
...
for MBT in 8192 32768; do
  TAG=base1xas_mbt${MBT}
  say "== mbt=$MBT: cleanup + start baseline (ASYNC_SCHEDULING=1) =="
  $S "$POD" -- bash /tmp/xnode32_cleanup.sh 2>&1 | tr -d '\r' | tail -2
  $S "$POD" -- bash -c "setsid bash -c 'env DP_SIZE=4 TP_SIZE=4 MAX_NUM_BATCHED_TOKENS=$MBT PORT=8000 ADDITIONAL_CONFIG=cws ASYNC_SCHEDULING=1 nohup bash $BASE_LAUNCHER > /tmp/${TAG}.log 2>&1 &' ; echo started" 2>&1 | tr -d '\r' | tail -1
  ...
  $S "$POD" -- bash -c "grep -c 'Enabled DeepSeek-V4 persistent compressor tails' /tmp/${TAG}.log || true; grep -c 'async.scheduling\|AsyncScheduling' /tmp/${TAG}.log || true" 2>&1 | tr -d '\r' | tail -2
...
say "== ALL_COMPLETE baseline async sweep =="
