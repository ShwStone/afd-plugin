#!/usr/bin/env bash
# Dual-node 32-card DSV4 AFD (DP6TP4 + DP8EP8), mbt=65536, token-split,
# opt-in prefill-token DPLB (attention_dplb_policy=prefill_token_sum).
# One server per invocation; replays formal_1 at 1x / 1.5x / 2x.
#
# Usage: xnode32_toklb_runs.sh
...
NASDIR=/a3_inference/itask/workdir/tq02357756/shwstone/xnode32_toklb_as
TAG=xnode32_toklb_as_mbt65536
...
ENV1="NODE1_IP=$NODE1_IP NODE2_IP=$NODE2_IP MAX_NUM_BATCHED_TOKENS=65536 HCCL_BUFFSIZE=4096 FLASHCOMM1=1 ASYNC_MOE_UBATCHING=true ASYNC_MOE_SPLIT=token ATTN_DPLB_POLICY=prefill_token_sum"
say "== start stack (mbt=65536, HCCL_BUFFSIZE=4096, token-split + prefill_token_sum DPLB, async-sched ON) =="
...
$S "$MASTER" -- bash -c "grep -c 'Enabled DeepSeek-V4 persistent compressor tails' /tmp/${TAG}_attn1.log || true; grep -o \"async_moe_ubatching[^,}]*\" /tmp/${TAG}_attn1.log | head -1; grep -o \"async_moe_split[^,}]*\" /tmp/${TAG}_attn1.log | head -1; grep -o \"attention_dplb_policy[^,}]*\" /tmp/${TAG}_attn1.log | head -1" 2>&1 | tr -d '\r' | tail -4
