...
# Launch the v0.26 Attention (DP2TP8 + FlashComm1) Async CAM recipe with
# precision capture for one execution mode. Run on the ATTENTION node; the FFN
# twin is launch_ffn_capture.sh.
#
...
#   bash tools/precision/launch_attn_capture.sh
#
...
# server log in $CAPTURE_DIR/attention.log.
set -u
...
echo "launched attention mode=$MODE pid=$! log=$LOG"
